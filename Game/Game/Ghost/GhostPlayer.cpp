...
void AGhostPlayer::Draw()
{
	...
	const int textWidth = GetDrawStringWidthToHandle(m_UserId.c_str(), static_cast<int>(m_UserId.length()), m_UserIdFontHandle);
	labelPos.X -= textWidth * 0.5f;
	labelPos.Y -= 72.0f;

	renderSystem.SubmitText({ labelPos.X + 1.0f, labelPos.Y + 1.0f }, m_UserId, m_UserIdFontHandle, 0x000000, RenderSpace::Screen, 2, 180);
	renderSystem.SubmitText(labelPos, m_UserId, m_UserIdFontHandle, 0xFFFFFF, RenderSpace::Screen, 3, 255);
}

void AGhostPlayer::SetUserId(const std::string& UserId)
{
	m_UserId = UserId;
}
