...
void WLobbyHUD::Draw() {
  ...
  ImGui::Begin("Lobby");
  ImGui::Text(
      "Mode: %s", world->IsListenServer() ? "Host" : (world->IsClient() ? "Client" : "Standalone")
  );
  ImGui::Text(
      "Players: %d / %d", static_cast<int>(states.size()), LobbyController->GetMaxPlayers()
  );
  ImGui::Separator();
  ...
  if (world->IsServer()) {
    ...
    const bool allReady =
        std::all_of(states.begin(), states.end(), [](const ALobbyPlayerState* state) {
